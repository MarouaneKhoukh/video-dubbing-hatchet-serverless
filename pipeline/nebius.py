...
async def create_nebius_job(
    ...
    platform = settings.gpu_platform if gpu else settings.cpu_platform
    preset = settings.gpu_preset if gpu else settings.cpu_preset
    ...
    spec = JobSpec(
        ...
        disk=JobSpec.DiskSpec(
            ...
            size_bytes=settings.job_disk_gb * 1024 * 1024 * 1024,
        ),
        volumes=[
            JobSpec.VolumeMount(
    ...
    logger.info(f"Creating Nebius job: {name} | gpu: {gpu} | preemptible: {preemptible}")
...
async def create_and_wait(
    ...
    job_id = await create_nebius_job(name, image, args, gpu, preemptible, container_command)
